...
type Props = {
  ...
  labels: {
    ...
    badge1: string;
    badge2: string;
    pickPackTitle: string;
    pickPackSub: string;
    packCta: (pack: string) => string;
...
export async function DarbhangaBundleLanding() {
  ...
  const [config, platform] = await Promise.all([
    loadDarbhangaConfig(locale),
    loadPlatformConfig(locale),
  ]);

  const labels = {
    badge0: t("darbhangaBadge0"),
    badge1: t("darbhangaBadge1"),
    badge2: t("darbhangaBadge2"),
    pickPackTitle: t("pickPackTitle"),
    pickPackSub: t("pickPackSub"),
    packCta: (pack: string) => t("packCta", { pack }),
    howItWorksTitle: t("howItWorksTitle"),
    testimonialSection: t("testimonialSection"),
    demoCall: t("demoCall"),
    orWhatsApp: t("orWhatsApp"),
    whatsappWrite: t("whatsappWrite"),
    bookNowHome: t("bookNowHome"),
  };

  return <DarbhangaBundleView config={config} platform={platform} labels={labels} />;
}

function DarbhangaBundleView({ config, platform, labels }: Props) {
  ...
  return (
    <MarketingShell>
      <div className="bg-gradient-to-b from-emerald-600 to-emerald-700 text-white">
        <div className="shell py-10 md:py-14">
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-[12px] font-semibold">
            <span className="rounded-full bg-white/20 px-3 py-1">{labels.badge0}</span>
            <span className="rounded-full bg-white/20 px-3 py-1">{labels.badge1}</span>
            <span className="rounded-full bg-white/20 px-3 py-1">{labels.badge2}</span>
          </div>
        </div>
      </div>

      <MarketingSection>
        <h2 className="text-[13px] font-bold uppercase tracking-widest text-zinc-400">{labels.pickPackTitle}</h2>
        <p className="mt-1 text-[15px] font-semibold text-zinc-800">{labels.pickPackSub}</p>
      ...
      <MarketingSection muted>
        <h2 className="text-[18px] font-bold text-zinc-900">{labels.howItWorksTitle}</h2>
      ...
      <MarketingSection>
        <h2 className="text-[18px] font-bold text-zinc-900">{labels.testimonialSection}</h2>
      ...
      <MarketingSection>
        <div className="text-center">
          <p className="text-[14px] text-zinc-600">{labels.demoCall}</p>
          <a
          ...
          <p className="mt-3 text-[12px] text-zinc-400">{labels.orWhatsApp}</p>
          <a
          ...
          >
            {labels.whatsappWrite}
          </a>
        </div>
      </MarketingSection>

      <div className="shell pb-8 text-center text-[11px] text-zinc-400">
        <a href="/" className="hover:underline">{labels.bookNowHome}</a>
        {" · "}
